...
# create a Dash App
app = dash.Dash(__name__, external_stylesheets = [dbc.themes.MATERIA], meta_tags=[dict(name = "viewport",meta_tags=[dict(name="viewport", content="width=device-width, initial-scale=1.0")],)])
...
def filtered_df(stock, time_index):
    # tuple unpack a list
    dff_daily, dff_intrady = df_dict[stock]

    dff = dff_intrady if time_index <= 2 else dff_daily
